...
export function PlayerController({
  ...
  useFrame((_state, delta) => {
    ...
    // PREDICT locally right now (instant response) using the shared step...
    stepPlayer(world, phys.current, input);
    // ...and queue the SAME input for the server to simulate authoritatively.
    ...
    if (sendInputs && sendAccum.current >= 0.05) {
      sendAccum.current = 0;
      sendInputs(outbox.current);
      outbox.current = [];
    }
