...
export function getVoxel(
  ...
  if (y >= w.sizeY || x < 0 || x >= w.sizeX || z < 0 || z >= w.sizeZ)
    return Block.Air;
...
export function setVoxel(
  ...
  if (y < 0 || y >= w.sizeY || x < 0 || x >= w.sizeX || z < 0 || z >= w.sizeZ)
    return;
...
export function generateWorld(
  ...
  for (let z = 0; z < sizeZ; z++) {
    for (let x = 0; x < sizeX; x++) {
      ...
      if (h <= w.waterLevel) {
        ...
        for (let y = h; y <= w.waterLevel; y++)
          setVoxel(w, x, y, z, Block.Water);
      } else {
        ...
        setVoxel(
          ...
          h - 1,
          z,
          h <= w.waterLevel + 1 ? Block.Sand : Block.Grass,
        );
      }
    }
  }
  ...
  for (let z = 3; z < sizeZ - 3; z++) {
    for (let x = 3; x < sizeX - 3; x++) {
      ...
      for (let dy = 1; dy <= trunk; dy++)
        setVoxel(w, x, sy + dy, z, Block.Wood);
      const cy = sy + trunk;
